class Api {
    ...
    sendApiRequest(parameters, callback) {
        ...
        if (this.api_coverage !== 'phone' && this.api_coverage !== 'both') {
            delete parameters.phone_number;
        }
        ...
        this.createHttpRequest(this.api_url, 'GET', parameters).then((httpRequest) => {
            ...
            if (responseCode !== 200) {
                ...
                console.log(`AUTHENTICITY ERROR: ${JSON.stringify(response)}`)
            }
            if (responseCode === 200) {
                if (response.email && this.api_coverage !== 'phone')
                    this.handleFieldResponse(this.#emailObject, response.email)
                else
                    this.#emailObject.error = false

                if (response.phone && this.api_coverage !== 'email')
    ...
    createClickSubmitEvents(email, phone, handler, type = 1, action = 'click') {
        if (handler) {
            ...
            const handlerFunction = (e) => {
                if (this.#emailObject.error === false && this.#phoneObject.error === false)
                    return;
                ...
                function dispatch() {
                    ...
                    if (action === 'submit')
                        handlerSelector.submit()
                    else
                        handlerSelector.click()
                }
            ...
            handlerSelector.addEventListener(action, handlerFunction);
        }
    }
    ...
    init(type, fields) {
        ...
        if (fields.email && (this.api_coverage === 'both' || this.api_coverage === 'email'))
            this.#emailObject = this.initializeField(this.#emailObject, fields.email, 'email')

        if (fields.phone && (this.api_coverage === 'both' || this.api_coverage === 'phone'))
